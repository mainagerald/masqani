...
import { LocationMapService } from '../../landlord/create-property/step/location-step/location-map.service';
...
export class DisplayListingComponent implements OnInit, OnDestroy {
  ...
  countyService = inject(LocationMapService);
    ...
    effect(() => {
      ...
      if (listingByPublicIdState.status === "OK") {
        ...
        if (this.listing) {
          ...
          this.countyService.getCountyByCode(this.listing.location)
            .subscribe({
              next: county => {
                if (this.listing) {
